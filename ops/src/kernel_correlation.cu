...
__global__ void kernel_correlation_kernel(int b, int n, int npoints, int l, int m, float sigma,
  ...
  for (int i = threadIdx.y; i < l; i += blockDim.y) {  // for some channel
    const float *temp_k = kernel + i * npoints * 3;
    for (int j = threadIdx.x; j < n; j += blockDim.x) {  // for some node
      const float *temp_p = points + j * npoints * 3;

      float sum = 0.;
      int n_neibor = 0;  //zhuyijie
      for (int k1 = 0; k1 < m; ++k1) {
        ...
        for (int k2 = 0; k2 < npoints; ++k2) {
          ...
          if(x*x+y*y+z*z<1e-6) continue;   //zhuyijie x,y,z =x_j-x_i, but j==i
          ++n_neibor;                       //zhuyijie
      ...
      outputs[i * n + j] = sum / n_neibor;  //zhuyijie
    }
  }
}
...
__global__ void kernel_correlation_grad_kernel(int b, int n, int npoints, int l, int m, float sigma,
  ...
  float coef = 1. / (sigma * sigma);  //zhuyijie
  ...
  for (int i = threadIdx.y; i < l; i += blockDim.y) {
    for (int j = threadIdx.x; j < m; j += blockDim.x) {
      ...
      for (int k1 = 0; k1 < n; ++k1) {
        ...
        gx += grad_outputs[i * n + k1] * sum1 / n_neibor;  //zhuyijie
        gy += grad_outputs[i * n + k1] * sum2 / n_neibor;
        gz += grad_outputs[i * n + k1] * sum3 / n_neibor;
      }
