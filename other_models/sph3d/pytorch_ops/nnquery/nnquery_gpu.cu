...
void BuildSphereNeighbor_kernel_launcher_fast(int B, int N, int M, int nnSample, float radius,
                                 const float* database, const float* query, int* nnIndex,
                                 int* nnCount, float* nnDist, cudaStream_t stream)
{
    cudaError_t err;
    dim3 blocks(32);  // blockIdx.x(col), blockIdx.y(row)
    dim3 threads(1024);
    cal_nn_binidx<<<blocks, threads, 0, stream>>>(B, N, M, nnSample, radius,
                                 ...
                                 const float* database, const float* query, int* nnIndex, int* nnCount, cudaStream_t stream)
{
    ...
    cal_nn_binidx_cube<<<blocks, threads, 0, stream>>>(B, N, M, gridSize, nnSample, length, database, query, nnIndex, nnCount);
    err = cudaGetLastError();
    if (cudaSuccess != err) {
      fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
      exit(-1);
