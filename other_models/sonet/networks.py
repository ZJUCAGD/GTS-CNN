...
from models.sonet.util import som
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        # first PointNet
        self.opt.surface_normal = False
        if self.opt.surface_normal == True:
            self.first_pointnet = PointResNet(6, [64, 128, 256], activation=self.opt.activation, normalization=self.opt.normalization,
                                              momentum=opt.bn_momentum, bn_momentum_decay_step=opt.bn_momentum_decay_step, bn_momentum_decay=opt.bn_momentum_decay)
        else:
            debugPrint(self.opt.surface_normal)
