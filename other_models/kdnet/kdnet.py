...
class KDNet(nn.Module):
    def __init__(self, num_classes = 40, input_channels=3, depth=11, device_id=0, initial_weights=True):
        ...
        self.optimizer = optim.Adam(self.parameters(), weight_decay=1e-3)
        self.schedule = optim.lr_scheduler.StepLR(self.optimizer, 30, 0.6)
    ...
    def forward(self, x, c):
        def kdconv(x, dim, featdim, sel, conv):
            ...
            # debugPrint(sel.size())
            #sel = Variable(sel + tmp.long()).view(-1,1) # TODO (32768, 1)
        ...
        # x = torch.transpose(x.view(-1,1024,32),dim0=2,dim1=1) ##[N,32,1024]
