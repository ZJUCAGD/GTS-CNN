...
class KDNet(nn.Module):
    ...
    def forward(self, x, c): ## x [N,3,2048]
        x = x.permute((0, 2, 1))
        #x = torch.transpose(x.view(-1, 32, 1024))
    ...
    #         offset = Variable(
    #             (torch.arange(0, batchsize) * dim * 3).repeat(dim, 1).transpose(1, 0).contiguous().long().view(-1, 1))
    #         sel = sel + offset
    #
    #         if x.is_cuda:
    #             sel = sel.cuda()
    ...
    def fit(self, dataloader, epoch, writer):
        ...
        for batch_idx, (cutdim_v, points_v, targets) in enumerate(dataloader):  # zhuyijie
            ...
            outputs = self.forward(points_v, cutdim_v)
            # outputs = self(inputs)
